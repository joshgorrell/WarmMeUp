import React, { useRef, useState, useCallback, useContext, useMemo, createContext } from 'react';
import {
  View,
  StyleSheet,
  TouchableOpacity,
...
const SLIDE_THEMES = [
  { glow: 'rgba(255,90,61,0.22)',   glow2: 'rgba(255,46,138,0.12)'  },
  { glow: 'rgba(255,46,138,0.20)',  glow2: 'rgba(255,90,61,0.10)'   },
  { glow: 'rgba(255,138,61,0.20)',  glow2: 'rgba(255,46,138,0.10)'  },
  { glow: 'rgba(255,70,20,0.22)',   glow2: 'rgba(255,138,61,0.12)'  },
  { glow: 'rgba(140,80,255,0.22)',  glow2: 'rgba(100,40,220,0.12)'  },
  { glow: 'rgba(255,46,138,0.20)',  glow2: 'rgba(255,138,61,0.12)'  },
  { glow: 'rgba(0,180,200,0.20)',   glow2: 'rgba(0,120,180,0.12)'   },
  { glow: 'rgba(0,200,150,0.18)',   glow2: 'rgba(0,180,200,0.10)'   },
  { glow: 'rgba(255,60,100,0.20)',  glow2: 'rgba(255,138,61,0.12)'  },
  { glow: 'rgba(255,90,61,0.22)',   glow2: 'rgba(255,46,138,0.18)'  },
];
...
const useCarousel = () => {
  ...
  if (!ctx) throw new Error('useCarousel must be used within OnboardingCarousel');
  return ctx;
};

// ─── Style factory ────────────────────────────────────────────────────────────
function makeStyles(rv: ResponsiveValues) {
  const { isShort, height: SH, slideWidth } = rv;
  ...
  return { fr, cp, vs, hl, pgb, sl, s };
}

type AllStyles = ReturnType<typeof makeStyles>;
...
function FeatureRow({
  ...
  return (
    <View style={[styles.fr.row, highlight && styles.fr.rowHL]}>
      <View style={[styles.fr.iconWrap, highlight && styles.fr.iconWrapHL]}>{icon}</View>
      <View style={styles.fr.texts}>
        <AppText style={[styles.fr.label, highlight && styles.fr.labelHL]}>{label}</AppText>
        {sub ? <AppText style={styles.fr.sub}>{sub}</AppText> : null}
      </View>
    </View>
  );
}
...
function CouplePill({ icon, label, color }: { icon: React.ReactNode; label: string; color: string }) {
  const { styles } = useCarousel();
  return (
    <View style={[styles.cp.pill, { borderColor: `${color}44` }]}>
      {icon}
      <AppText style={[styles.cp.label, { color }]}>{label}</AppText>
    </View>
  );
}
...
function VisualWelcome() {
  ...
  return (
    <View style={styles.vs.center}>
      <Animated.View style={[styles.vs.welcomeOuterGlow, { opacity: fade, transform: [{ scale: pulse }] }]} />
      <Animated.View style={[styles.vs.welcomeInnerRing, { opacity: fade }]} />
      <WarmupLogo size={sz} />
    ...
    <View style={styles.vs.center}>
      <View style={styles.vs.whyGrid}>
        {([
          ...
          <View key={i} style={[styles.vs.whyChip, { borderColor: `${color}38` }]}>
            <Icon color={color} size={20} strokeWidth={1.8} />
            <AppText style={[styles.vs.whyChipLabel, { color: `${color}CC` }]}>{label}</AppText>
          </View>
        ))}
      </View>
      <View style={styles.vs.whyTagPill}>
        <AppText style={styles.vs.whyTagText}>More intimate than social media</AppText>
      </View>
    </View>
  );
}
...
function VisualChat() {
  const { styles } = useCarousel();
  return (
    <View style={[styles.vs.center, { gap: 9, paddingHorizontal: 20 }]}>
      <View style={styles.vs.chatBadge}>
        <Lock color="#FF8A3D" size={11} strokeWidth={2.5} />
        <AppText style={styles.vs.chatBadgeText}>Couple-only chat</AppText>
      </View>
      <View style={[styles.vs.bubble, styles.vs.bubbleLeft]}>
        <AppText style={styles.vs.bubbleText}>What are you thinking? 💭</AppText>
      </View>
      <View style={[styles.vs.bubble, styles.vs.bubbleRight]}>
        <AppText style={styles.vs.bubbleText}>Only you would know... 🔒</AppText>
      </View>
      <View style={[styles.vs.bubble, styles.vs.bubbleLeft]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <Mic color="#FF8A3D" size={13} strokeWidth={2} />
          <AppText style={styles.vs.bubbleText}>Voice note  0:08</AppText>
        </View>
      </View>
    </View>
  );
...
function VisualDare() {
  ...
  return (
    <View style={styles.vs.center}>
      <Animated.View style={[styles.vs.dareGlow, { opacity: glow }]} />
      <View style={styles.vs.dareRing}>
        <Flame color="#FF8A3D" size={sz} strokeWidth={1.4} />
      </View>
      <View style={styles.vs.dareCards}>
        <View style={[styles.vs.dareCard, { borderColor: 'rgba(255,138,61,0.38)' }]}>
          <AppText style={[styles.vs.dareWord, { color: 'rgba(255,255,255,0.68)' }]}>Truth</AppText>
          <AppText style={[styles.vs.dareWord, { color: 'rgba(255,255,255,0.42)', fontSize: 11 }]}>or</AppText>
          <AppText style={[styles.vs.dareWord, { color: '#FF8A3D' }]}>Dare?</AppText>
          <AppText style={styles.vs.dareQ}>?</AppText>
        </View>
        <View style={[styles.vs.dareCard, { borderColor: 'rgba(255,46,138,0.42)', backgroundColor: 'rgba(255,46,138,0.08)' }]}>
          <AppText style={[styles.vs.dareWord, { color: 'rgba(255,255,255,0.68)', fontSize: 11 }]}>Accept</AppText>
          <AppText style={[styles.vs.dareWord, { color: '#FF2E8A' }]}>the dare?</AppText>
          <View style={styles.vs.dareCheck}>
...
function VisualDice() {
  ...
  const DFace = ({ angle, style }: { angle: string; style?: object }) => (
    <Animated.View style={[styles.vs.diceOuter, style, { transform: [{ rotate: angle }] }]}>
      <LinearGradient colors={['rgba(140,80,255,0.42)', 'rgba(70,30,180,0.32)']} style={styles.vs.diceFace}>
        <View style={styles.vs.pipRow}><View style={styles.vs.pip} /><View style={{ width: 8 }} /><View style={styles.vs.pip} /></View>
        <View style={styles.vs.pipRow}><View style={{ width: 8 }} /><View style={styles.vs.pip} /><View style={{ width: 8 }} /></View>
        <View style={styles.vs.pipRow}><View style={styles.vs.pip} /><View style={{ width: 8 }} /><View style={styles.vs.pip} /></View>
      </LinearGradient>
    </Animated.View>
  );
  return (
    <View style={styles.vs.center}>
      <View style={styles.vs.diceGlowRing} />
      <View style={styles.vs.diceGroup}>
...
function VisualWish() {
  ...
  const { isShort } = rv;
  return (
    <View style={[styles.vs.center, { paddingHorizontal: 16 }]}>
      <View style={styles.vs.wishRing}>
        <Gift color="#FF2E8A" size={isShort ? 42 : 52} strokeWidth={1.4} />
      </View>
      <View style={styles.vs.polaroidStack}>
        <View style={[styles.vs.polaroid, styles.vs.polBack]}>
          <LinearGradient colors={['#1a0e18', '#2a1428']} style={styles.vs.polImg} />
          <AppText style={styles.vs.polCaption}>A Weekend Away ❤️</AppText>
        </View>
        <View style={[styles.vs.polaroid, styles.vs.polFront]}>
          <LinearGradient colors={['#18122a', '#0e1020']} style={styles.vs.polImg}>
            <MapPin color="rgba(255,138,61,0.75)" size={18} strokeWidth={1.8} />
          </LinearGradient>
          <AppText style={styles.vs.polCaption}>Someday in Paris 🗼</AppText>
        </View>
      </View>
      <View style={styles.vs.wishTagRow}>
        <Heart color="#FF2E8A" size={11} strokeWidth={2} fill="#FF2E8A" />
        <AppText style={styles.vs.wishTagText}>Some wishes become unforgettable memories ❤️</AppText>
      </View>
    </View>
  );
}
...
function VisualVault() {
  ...
  return (
    <View style={styles.vs.center}>
      <Animated.View style={[styles.vs.vaultOuterRing, { opacity: glow, transform: [{ scale: ringS }] }]} />
      <Animated.View style={[styles.vs.vaultInnerRing, { opacity: glow }]} />
      <View style={styles.vs.vaultBox}>
        <Lock color="#00B4C8" size={sz} strokeWidth={1.4} />
      </View>
      <View style={styles.vs.vaultPill}>
        <Shield color="#00B4C8" size={11} strokeWidth={2} />
        <AppText style={styles.vs.vaultPillText}>Encrypted & private</AppText>
      </View>
    </View>
  );
}
...
function VisualSettings() {
  ...
  return (
    <View style={[styles.vs.center, { width: '100%', paddingHorizontal: 20 }]}>
      <View style={styles.vs.settingsAvatar}>
        <User color="rgba(255,255,255,0.6)" size={28} strokeWidth={1.6} />
      </View>
      <AppText style={styles.vs.settingsAvatarLabel}>Your Profile</AppText>
      <View style={styles.vs.settingsMock}>
        {rows.map(({ Icon, color, label }, i) => (
          <View key={i} style={[styles.vs.settingsRow, i < rows.length - 1 && styles.vs.settingsRowBorder]}>
            <View style={[styles.vs.settingsIconWrap, { backgroundColor: `${color}1E` }]}>
              <Icon color={color} size={13} strokeWidth={2} />
            </View>
            <AppText style={styles.vs.settingsRowLabel}>{label}</AppText>
...
function VisualEveryCouple() {
  ...
  return (
    <View style={styles.vs.center}>
      <Animated.View style={[styles.vs.coupleGlow, { transform: [{ scale }] }]} />
      ...
      <AppText style={styles.vs.coupleLabel}>WarmMeUp is for every couple</AppText>
    </View>
  );
}
...
function VisualFinalCTA() {
  ...
  return (
    <View style={styles.vs.center}>
      <Animated.View style={[styles.vs.finalGlowOuter, { opacity: glow, transform: [{ scale }] }]} />
      <Animated.View style={[styles.vs.finalGlowInner, { opacity: glow }]} />
...
function Headline({ text, accentWord, accentColor }: { text: string; accentWord?: string; accentColor: string }) {
  const { styles } = useCarousel();
  if (!accentWord) return <AppText style={styles.hl.h}>{text}</AppText>;
  const idx = text.indexOf(accentWord);
  if (idx === -1) return <AppText style={styles.hl.h}>{text}</AppText>;
  return (
    <AppText style={styles.hl.h}>
      {text.slice(0, idx)}
      <AppText style={[styles.hl.h, { color: accentColor }]}>{accentWord}</AppText>
...
function ProgressBar({ current, total }: { current: number; total: number }) {
  const { styles } = useCarousel();
  return (
    <View style={styles.pgb.wrap}>
      {Array.from({ length: total }).map((_, i) => (
        <View key={i} style={[styles.pgb.seg, i < current ? styles.pgb.done : i === current ? styles.pgb.active : styles.pgb.empty]}>
          {i === current && (
            <LinearGradient
...
function Slide({ item, width, visualH }: { item: SlideData; width: number; visualH: number }) {
  ...
  const { Visual } = item;
  return (
    <View style={[styles.sl.slide, { width }]}>
      <View style={styles.sl.slideInner}>
        <View style={[styles.sl.visual, { height: visualH }]}>
          <Visual />
        </View>
        <View style={styles.sl.content}>
          {item.posterTitle ? (
            <AppText style={[styles.sl.posterTitle, { color: item.accentColor }]}>{item.posterTitle}</AppText>
          ) : null}
          <Headline text={item.headline} accentWord={item.headlineAccent} accentColor={item.accentColor} />
          {item.subtext ? <AppText style={styles.sl.subtext}>{item.subtext}</AppText> : null}
          {item.rows.length > 0 && (
            <View style={styles.sl.rows}>
              {item.rows.map((row, i) => (
          ...
          {item.couplePills && (
            <View style={styles.sl.pillsWrap}>
          ...
          {item.isFinal && (
            <View style={styles.sl.stayPlayfulWrap}>
              <Image source={SLOGAN_SOURCE} style={styles.sl.stayPlayfulImg} resizeMode="contain" />
            </View>
          )}
        </View>
      </View>
...
export default function OnboardingCarousel({ mode, onComplete }: Props) {
  ...
  const isShort = layout.height < 700;
  const slideWidth = layout.isTabletOrLarger ? Math.min(layout.width, 480) : layout.width;

  const rv = useMemo<ResponsiveValues>(() => ({
    isShort,
    width: layout.width,
    height: layout.height,
    slideWidth,
    isTabletOrLarger: layout.isTabletOrLarger,
  }), [isShort, layout.width, layout.height, slideWidth, layout.isTabletOrLarger]);

  const styles = useMemo(() => makeStyles(rv), [rv]);
  const ctxValue = useMemo(() => ({ rv, styles }), [rv, styles]);
  ...
  const visualH = Math.round(
    (currentSlide.wideVisual
      ? (isShort ? 0.44 : 0.50)
      : (isShort ? 0.38 : 0.43)) * layout.height
  );

  return (
    <CarouselContext.Provider value={ctxValue}>
      <View style={styles.s.root}>
        <LinearGradient colors={['#060406', '#08060A', '#0C080C']} style={StyleSheet.absoluteFill} />
        <Animated.View style={[styles.s.ambientTop, { backgroundColor: topGlow }]} pointerEvents="none" />
        <Animated.View style={[styles.s.ambientBottom, { backgroundColor: bottomGlow }]} pointerEvents="none" />

        {/* Header */}
        <View style={[styles.s.header, { paddingTop: topPad }]}>
          <AppText style={styles.s.counter}>{currentIndex + 1} / {SLIDES.length}</AppText>
          <ProgressBar current={currentIndex} total={SLIDES.length} />
          <TouchableOpacity
            style={styles.s.skipBtn}
            onPress={() => onComplete('get-started')}
            activeOpacity={0.7}
            hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
          >
            <AppText style={styles.s.skipText}>Skip</AppText>
          </TouchableOpacity>
        </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) => <Slide item={item} width={layout.width} visualH={visualH} />}
          horizontal
          pagingEnabled
          ...
          extraData={`${layout.width}-${layout.height}`}
          style={styles.s.list}
        ...
        {/* Footer */}
        <View style={[styles.s.footer, { paddingBottom: bottomPad }]}>
          {currentSlide.isFinal ? (
            <>
              <TouchableOpacity style={styles.s.primaryBtn} onPress={() => onComplete('get-started')} activeOpacity={0.87}>
                <LinearGradient
                  colors={['#FFB347', '#FF5A3D', '#FF2E8A']}
                  start={{ x: 0, y: 0 }}
                  end={{ x: 1, y: 0 }}
                  style={styles.s.primaryGrad}
                >
                  <Sparkles color="#fff" size={15} strokeWidth={2.2} />
                  <AppText style={styles.s.primaryLabel}>Get Started</AppText>
                </LinearGradient>
              </TouchableOpacity>
              <TouchableOpacity style={styles.s.secondaryBtn} onPress={() => onComplete('invite-partner')} activeOpacity={0.8}>
                <UserPlus color="rgba(255,255,255,0.72)" size={15} strokeWidth={2} />
                <AppText style={styles.s.secondaryLabel}>Invite Your Partner</AppText>
              </TouchableOpacity>
            </>
          ) : (
            <TouchableOpacity style={styles.s.primaryBtn} onPress={handleNext} activeOpacity={0.87}>
              <LinearGradient
                colors={['#FFB347', '#FF5A3D', '#FF2E8A']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={styles.s.primaryGrad}
              >
                <AppText style={styles.s.primaryLabel}>Next</AppText>
              </LinearGradient>
            </TouchableOpacity>
          )}
        </View>
