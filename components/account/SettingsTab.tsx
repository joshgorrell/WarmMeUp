  ...
  return (
    <>
      ...
      <Section title="MEMBERSHIP">
        {subscriptionInfo.loading ? <SettingsRow label="Status" sub="Loading…" last /> : subscriptionInfo.source === 'review_access' ? <SettingsRow label="Access" sub="App Review Demo Access" last /> : (subscriptionInfo.source === 'admin' || subscriptionInfo.source === 'super_admin') ? <SettingsRow label="Access" sub="Admin Access" last /> : subscriptionInfo.source === 'partner' ? <><SettingsRow label="Plan" sub="Covered by partner's subscription" /><SettingsRow label="Manage" sub="View or cancel in the App Store" onPress={() => Linking.openURL('https://apps.apple.com/account/subscriptions')} accent last /></> : subscriptionInfo.source === 'self' && subscriptionInfo.isOnTrial ? <><SettingsRow label="Plan" sub={`Free Trial${subscriptionInfo.trialExpiresAt ? ` · ends ${new Date(subscriptionInfo.trialExpiresAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}` : ''}`} /><SettingsRow label="Subscribe" sub="Unlock full access · your partner joins free" onPress={() => router.push('/(auth)/subscription')} accent /><SettingsRow label="Restore Purchase" sub="Recover a previous subscription" onPress={onRestorePurchase} last /></> : subscriptionInfo.source === 'self' && subscriptionInfo.isPremium ? <><SettingsRow label="Plan" sub={`${subscriptionInfo.plan === 'yearly' ? 'Yearly' : 'Monthly'} · Active${subscriptionInfo.expiresAt ? ` — active through ${new Date(subscriptionInfo.expiresAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}` : ''}`} /><SettingsRow label="Manage Subscription" sub="View or cancel in the App Store" onPress={() => Linking.openURL('https://apps.apple.com/account/subscriptions')} accent /><SettingsRow label="Restore Purchase" sub="Recover a previous subscription" onPress={onRestorePurchase} last /></> : <><SettingsRow label="Plan" sub="No active subscription" /><SettingsRow label="Subscribe" sub="One subscription covers both of you · partner joins free" onPress={() => router.push('/(auth)/subscription')} accent /><SettingsRow label="Restore Purchase" sub="Recover a previous subscription" onPress={onRestorePurchase} last /></>}
      </Section>
