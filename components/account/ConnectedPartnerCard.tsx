      ...
      >
        <View style={pcc.inner}>
          ...
          <View style={pcc.avatarRow}>
            <Avatar name={userProfile?.display_name} uri={userProfile?.avatar_url} size="lg" />
            <View style={pcc.heartZone}>
              <LinearGradient colors={['transparent', 'rgba(255,46,138,0.45)', 'transparent']} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={pcc.waveLine} />
              <Animated.View style={[pcc.heartWrap, { transform: [{ scale: pulseAnim }] }]}>
                <LinearGradient colors={['rgba(255,90,61,0.28)', 'rgba(255,46,138,0.28)']} style={pcc.heartGlowBg} />
                <Heart color="#FF2E8A" size={38} strokeWidth={0} fill="#FF3D6A" />
              </Animated.View>
              <LinearGradient colors={['transparent', 'rgba(255,46,138,0.45)', 'transparent']} start={{ x: 0, y: 0.5 }} end={{ x: 1, y: 0.5 }} style={pcc.waveLine} />
            </View>
            <Avatar name={partner?.display_name} uri={partner?.avatar_url} size="lg" />
          </View>
...
const pcc = StyleSheet.create({
  ...
  inner: { borderRadius: Radius.xl, backgroundColor: 'rgba(18,12,26,0.97)', padding: Spacing.card, gap: 14, overflow: 'hidden' },
  avatarRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  heartZone: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
  waveLine: { flex: 1, height: 2, borderRadius: 1 },
  heartWrap: { width: 64, height: 64, alignItems: 'center', justifyContent: 'center' },
  heartGlowBg: { ...StyleSheet.absoluteFillObject, borderRadius: 32 } as any,
  textBlock: { alignItems: 'center', gap: 4 },
  connectedWithLabel: { fontSize: 11, fontFamily: 'Inter-SemiBold', letterSpacing: 1.4, color: '#FF5A3D' },
  partnerName: { fontSize: 32, fontFamily: 'Inter-Bold', color: '#FFFFFF', lineHeight: 38 },
  tagline: { fontSize: FontSize.sm, fontFamily: 'Inter-Regular', color: 'rgba(255,255,255,0.50)', marginTop: 2 },
  statusRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 2 },
  connectedPill: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(51,209,122,0.12)', borderWidth: 1, borderColor: 'rgba(51,209,122,0.28)', borderRadius: Radius.pill, paddingHorizontal: 12, paddingVertical: 6 },
  greenDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#33D17A' },
  connectedPillText: { fontSize: FontSize.sm, fontFamily: 'Inter-SemiBold', color: '#33D17A' },
  manageCta: { flexDirection: 'row', alignItems: 'center', gap: 3, paddingVertical: 4 },
  manageCtaText: { fontSize: FontSize.sm, fontFamily: 'Inter-SemiBold', color: '#FF2E8A' },
  metricsCard: { flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.055)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', borderRadius: Radius.lg, paddingVertical: Spacing.md },
  metricCol: { flex: 1, alignItems: 'center', gap: 4 },
  metricDivider: { width: 1, alignSelf: 'stretch', backgroundColor: 'rgba(255,255,255,0.09)', marginVertical: 4 },
  metricValue: { fontSize: FontSize.h2, fontFamily: 'Inter-Bold', color: '#FFFFFF', lineHeight: 28 },
  metricLabel: { fontSize: 10, fontFamily: 'Inter-Medium', color: 'rgba(255,255,255,0.44)', textAlign: 'center', lineHeight: 14 },
  diceIconGrad: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
});
