...
// Discreet mode still identifies the kind of activity, but never exposes partner
// names, message text, emoji/reaction content, captions, or media previews.
const DISCREET_EVENT_LABELS: Record<string, string> = {
  new_message: "New message",
  new_photo: "New photo",
  new_video: "New video",
  new_vault_item: "New Vault activity",
  new_dare: "New dare",
  dare_accepted: "Dare update",
  dare_rejected: "Dare update",
  dare_completed: "Dare update",
  new_ask: "New question",
  ask_answered: "Question update",
  new_wish: "New wish",
  wish_bumped: "Wish update",
  wish_fulfilled: "Wish update",
  dice_roll: "New dice challenge",
  dice_accepted: "Dice update",
  dice_completed: "Dice update",
  send_love: "New activity",
};
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { event_type, couple_id, target_route, item_id, emoji, message_text } = body as {
      event_type: string; couple_id: string; target_route?: string; item_id?: string; emoji?: string; message_text?: string;
    };
    ...
    const bodyText = isDiscreet
      ? (DISCREET_EVENT_LABELS[event_type] ?? "New activity")
      : showText
        ? `${senderName}: ${trimmedText.slice(0, 200)}`
        : `${senderName} ${eventLabel}${suffix}`;

    const expoPayload = { to: partnerProfile.push_token, title, body: bodyText, data: { event_type, couple_id, target_route: target_route ?? null, item_id: item_id ?? null }, sound: "default" };
    console.log("[notify-partner] Sending push payload:", JSON.stringify({ to_prefix: (partnerProfile.push_token ?? "").slice(0, 30) + "...", title: expoPayload.title, body: expoPayload.body, sound: expoPayload.sound, data: expoPayload.data }));

    const pushRes = await fetch("https://exp.host/--/api/v2/push/send", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(expoPayload) });
    let ticket: any = null; let ticketId: string | null = null;
    try { const pushJson = await pushRes.json() as any; ticket = pushJson?.data ?? null; ticketId = ticket?.id ?? null; console.log("[notify-partner] Expo ticket:", JSON.stringify(ticket)); }
    catch (e: any) { console.error("[notify-partner] Failed to parse Expo push response:", e?.message ?? String(e)); }
    ...
    } else if (!pushRes.ok) console.error(`[notify-partner] Expo push HTTP ${pushRes.status} — non-ok response`);
