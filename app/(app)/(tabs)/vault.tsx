...
export default function VaultScreen() {
  ...
  const uploadToVault = async (localUri: string, mediaType: 'photo' | 'video', mimeType: string) => {
    ...
    if (!couple?.id) {
      if (subscriptionInfo.canInvite) {
        ...
        try {
          ...
          const insertPayload = { couple_id: rpcResult.couple_id, uploaded_by_user_id: user.id, media_type: mediaType, file_path: actualPath, storage_path: actualPath, storage_bucket: 'vault', blurred_thumbnail_path: uploadResult.thumbnailPath ?? null, allow_screenshot: settings?.vault_allow_screenshot_default ?? false, allow_save: settings?.vault_allow_save_default ?? false, allow_share: settings?.vault_allow_share_default ?? false, chat_message_id: null };
    ...
    try {
      ...
      const actualPath = uploadResult.storagePath;
      const insertPayload = { couple_id: couple.id, uploaded_by_user_id: user.id, media_type: mediaType, file_path: actualPath, storage_path: actualPath, storage_bucket: 'vault', blurred_thumbnail_path: uploadResult.thumbnailPath ?? null, allow_screenshot: settings?.vault_allow_screenshot_default ?? false, allow_save: settings?.vault_allow_save_default ?? false, allow_share: settings?.vault_allow_share_default ?? false, chat_message_id: null };
      const { error: dbError } = await supabase.from('vault_items').insert(insertPayload);
      if (dbError) { supabase.storage.from('vault').remove([actualPath]).catch(() => {}); throw new Error(`Media uploaded but failed to save — ${dbError.message}`); }
        ...
        {items.length === 0 ? <View style={styles.empty}><View style={styles.emptyIconWrap}><Shield color="rgba(255,255,255,0.20)" size={48} /></View><AppText style={[styles.emptyTitle, { color: colors.text }]}>Nothing yet</AppText><AppText style={[styles.emptySub, { color: colors.textSecondary }]}>Something new is waiting. Add your first private moment.</AppText></View> :
          <View style={styles.grid}>{items.map(item => {
            ...
            return <View key={item.id} style={{ position: 'relative' }}><TouchableOpacity ref={ref => { tileRefs.current[item.id] = ref as any; }} style={[styles.gridItem, { width: ITEM_SIZE, height: ITEM_SIZE }]} onPress={() => selectMode ? toggleSelection(item.id) : handleTilePress(item)} onLongPress={() => selectMode ? undefined : handleVaultLongPress(item)} delayLongPress={400} activeOpacity={0.85}>
              {url ? <Image source={{ uri: url }} style={[StyleSheet.absoluteFill, { borderRadius: Radius.sm }]} contentFit="cover" cachePolicy="memory-disk" transition={120} /> : <View style={[StyleSheet.absoluteFill, styles.gridPlaceholder]}><ActivityIndicator color="rgba(255,255,255,0.25)" size="small" /></View>}
