...
import { Heart, Trash2, TriangleAlert as AlertTriangle } from 'lucide-react-native';
...
type AdminGrantRow = { id: string; user_id: string; entitlement_type: string; expires_at: string | null; active: boolean; can_invite: boolean };
type EffectiveEntitlement = { label: string; source: 'super_admin' | 'admin' | 'self' | 'trial' | 'admin_grant' | 'partner' | 'none'; isActive: boolean };
type SelectedUser = { profile: ProfileRow; couple: CoupleRow | null; partnerName: string | null; subscription: SubscriptionRow | null; grant: AdminGrantRow | null; entitlement: EffectiveEntitlement };
...
export default function UsersDashboard() {
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [profilesResult, couplesResult, subscriptionsResult, grantsResult] = await Promise.all([
        ...
        supabase.from('admin_grants').select('id, user_id, entitlement_type, expires_at, active, can_invite').eq('active', true).order('created_at', { ascending: false }),
      ]);
  ...
  const grantByUser = useMemo(() => {
    ...
    grants.forEach(g => { if (!map[g.user_id]) map[g.user_id] = g; });
    return map;
  }, [grants]);

  const isGrantActive = (grant: AdminGrantRow | null) => !!grant?.active && (!grant.expires_at || new Date(grant.expires_at) >= new Date());
  ...
  const computeEntitlement = (profile: ProfileRow): EffectiveEntitlement => {
    ...
    if (isGrantActive(grant)) {
      const typeLabel = grant!.entitlement_type === 'free_access' ? 'Free Access' : grant!.entitlement_type === 'extended_trial' ? 'Extended Trial' : grant!.entitlement_type === 'comped_subscription' ? 'Comped Subscription' : grant!.entitlement_type;
    ...
    if (couple && couple.active !== false) {
      ...
      if (partnerId) {
        ...
        if (isGrantActive(grantByUser[partnerId] ?? null)) return { label: 'Covered by partner (grant)', source: 'partner', isActive: true };
      }
    }
  ...
  const openUser = (profile: ProfileRow) => {
    ...
    setSelectedUser({ profile, couple, partnerName: partnerId ? safeName(profileById[partnerId]) : null, subscription: subscriptionByUser[profile.id] ?? null, grant: grantByUser[profile.id] ?? null, entitlement: computeEntitlement(profile) });
  };
    ...
    return (
      <View style={styles.root}>
        <ScrollView contentContainerStyle={[styles.content, isWideScreen && styles.contentWide, { paddingTop: Math.max(insets.top + 20, 32) }]}>
          ...
          {grant && <><DetailCard label="ADMIN GRANT" value={grant.entitlement_type} /><DetailCard label="GRANT EXPIRES" value={fmtDate(grant.expires_at)} /><DetailCard label="GRANT CAN INVITE" value={grant.can_invite ? 'Yes' : 'No'} /></>}
