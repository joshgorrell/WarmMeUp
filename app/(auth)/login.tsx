...
export default function LoginScreen() {
  ...
  const handleOAuth = async (provider: 'apple' | 'google') => {
    ...
    try {
      ...
      // Web redirects away (session = null); native returns a session
      if (!session) return;

      const userId = session.user?.id;
      if (userId) {
        // The DB trigger auto-creates profile + user_settings rows.
        // Determine if this user has been through setup (profile existed before this sign-in).
